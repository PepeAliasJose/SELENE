...
while True:
    ...
    if clase==1 and p>=0.65 and fb==0: #if para mandar una foto si detecta un brazo y parar la instalacion
        ...
        tb.send_message(chatid,'Instalación parada. Persona detectada. ERR:001')
    ...
    if clase==2 and p>=0.55 and Y<40:
        GPIO.output(18,1)
        print("Rueda en cinta.")
    if clase==2 and p>=0.55 and X>=136 and Y<40:
        GPIO.output(23,1)
        GPIO.output(18,0)
        print("Rueda en posición.")
    if clase==2 and p>=0.55 and Y>120:
        GPIO.output(13,1)
        GPIO.output(23,0)
        print("FANUC va a por la rueda.")
    if clase!=2 or p<=0.5:
        GPIO.output(13,0)
        GPIO.output(18,0)
        GPIO.output(23,0)
    ...
    print("FPS:",fps)
    cv2.imshow('SELENE',cv2.resize(image_rgb_np_with_detections,(600,400)))
